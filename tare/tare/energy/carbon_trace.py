...
**Status**: carbon is a *derived* metric in HASAGI, not a measured one. Use these traces to
report carbon proxy alongside the primary kWh measurement; never as the sole control signal.
See ``tare.energy.carbon_sources`` for the live multi-source proxy and its uncertainty band.
...
# The pre-registered cross-zone analysis uses the 16 zones above. A coverage
# expansion to 30 zones (14 more ElectricityMaps zones spanning four further
# continents and the full clean-to-dirty / flat-to-swingy range) is run as an
# explicit robustness check, not the default; its real CSVs live in
# data_cache/real_traces{,_winter}/ and the zone ids are listed in
# experiments/exp_realtrace_pareto.py (EXPANSION_ZONES).
