...
Run with: ``uvicorn tare.orchestrator.api:app --host 0.0.0.0 --port 8000``

The orchestrator runs the ``ControlLoop.tick`` in a background asyncio task on a fixed
cadence (``HASAGI_TICK_SECONDS``).
"""
...
from tare.config import SETTINGS
from tare.energy.carbon_trace import CarbonTrace, load_csv_trace, synthetic_solar_trace
from tare.energy.policy import RuleBasedPolicy
from tare.orchestrator.control_loop import ControlLoop, admit_or_drop
from tare.orchestrator.job import Job, JobState, JobStore
from tare.parallel.planner import SimpleRuntimeModel

logger = logging.getLogger("tare.orchestrator")
logging.basicConfig(level=SETTINGS.log_level)


JOBS_TOTAL = Counter("tare_jobs_submitted_total", "Number of jobs submitted")
JOBS_ADMITTED = Counter("tare_jobs_admitted_total", "Number of jobs admitted")
JOBS_DROPPED = Counter("tare_jobs_dropped_total", "Number of jobs dropped at admission")
CARBON_GAUGE = Gauge("tare_carbon_intensity_g_per_kwh", "Current carbon intensity")
...
app = FastAPI(title="HASAGI Orchestrator", lifespan=lifespan)
...
def run() -> None:
    """Entry point for ``tare-orchestrator`` console script."""
    import uvicorn
    uvicorn.run("tare.orchestrator.api:app", host="0.0.0.0", port=8000, reload=False)
