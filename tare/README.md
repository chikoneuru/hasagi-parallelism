...
```
tare/                  Python package (the framework)
...
```bash
kubectl apply -f k8s/
tare-cli submit --model resnet18 --dataset cifar10 \
...
```bash
make lint               # ruff check tare tests experiments
make test               # pytest -ra
.venv/bin/pytest -x     # fast-fail mode
.venv/bin/mypy tare/    # type check (informational)
```
