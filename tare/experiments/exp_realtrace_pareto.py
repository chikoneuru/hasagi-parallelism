...
#: Pre-registered cross-zone coverage expansion (an explicit robustness check, not
#: the default). These 14 ElectricityMaps zones add four further continents and the
#: full clean-to-dirty / flat-to-swingy range to the 16 in GRID_ZONE_IDS; real CSVs
#: ship in data_cache/real_traces{,_winter}/. Run with
#: ``--zones $(python -c "from experiments.exp_realtrace_pareto import ALL_ZONES_30 as z; print(*z)")``.
EXPANSION_ZONES: tuple[str, ...] = (
    "US-TEX", "US-PJM", "CA-ON", "ES", "IT", "NL", "SE",
    "DK", "IE", "TW", "TH", "ID", "MX", "CL",
)
ALL_ZONES_30: tuple[str, ...] = tuple(GRID_ZONE_IDS) + EXPANSION_ZONES
...
def run(args: argparse.Namespace) -> int:
    ...
    ztraces = load_zone_traces(args.real_dir, args.zones or list(GRID_ZONE_IDS))
...
def main() -> int:
    ...
    p.add_argument("--zones", nargs="+", default=None,
                   help="Explicit zone id list (default: the 16 pre-registered GRID_ZONE_IDS). "
                        "Pass ALL_ZONES_30 for the cross-zone coverage robustness check.")
