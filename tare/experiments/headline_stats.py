...
unified statistical report so every headline table in ``paper/tare/`` can cite
...
Usage::
    ...
    python -m experiments.headline_stats \\
        ...
        --tightbudget artifacts/tare_eb_tight_budget.json \\
...
def _analyse_scheduler_head_to_head(path: Path) -> dict[str, Any]:
    ...
    tare_by_seed = by_alloc["HASAGI EB"]
    seeds = sorted(tare_by_seed.keys())
    ...
    for name in sorted(by_alloc.keys()):
        if name == "HASAGI EB":
            continue
        diffs = [tare_by_seed[s] - by_alloc[name][s] for s in seeds]
        tare_e = [tare_by_seed[s] for s in seeds]
        base_e = [by_alloc[name][s] for s in seeds]
        mean, lo, hi = paired_bootstrap_ci(diffs, n_boot=N_BOOT, rng=rng)
        d = cohens_d(tare_e, base_e)
        p = paired_permutation_pvalue(tare_e, base_e, n_perm=N_PERM, rng=rng)
        rows.append({
            "baseline": name,
            "n_seeds": len(seeds),
            "delta_mean_kwh": mean,
...
def _analyse_tight_budget(path: Path) -> dict[str, Any]:
    ...
    for mult in multipliers:
        tare = by_key[(mult, "HASAGI EB")]
        pf = by_key[(mult, "PowerFlow")]
        e_diffs = [tare[s]["energy_kwh"] - pf[s]["energy_kwh"] for s in seeds]
        met_diffs = [tare[s]["deadlines_met"] - pf[s]["deadlines_met"] for s in seeds]
        ...
        d = cohens_d(
            [tare[s]["energy_kwh"] for s in seeds],
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default=Path("artifacts/tare_eb_tight_budget.json"),
    )
    parser.add_argument(
