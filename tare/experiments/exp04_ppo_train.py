"""Train a PPO scaling policy against the carbon-aware TareCarbonEnv.
...
Pipeline:
    ...
    3. Save the trained model to ``artifacts/ppo_tare.zip`` (gitignored).
...
from tare.energy.rl_env import EnvConfig, TareCarbonEnv


def make_env(cfg: EnvConfig, trace: CarbonTrace, seed: int = 0):
    def _thunk():
        env = TareCarbonEnv(cfg, trace=trace, seed=seed)
        return env
    return _thunk
...
def train(args: argparse.Namespace) -> Path:
    ...
    n_envs = args.n_envs
    vec_env = DummyVecEnv(
        [lambda i=i: Monitor(TareCarbonEnv(cfg, trace=trace, seed=i)) for i in range(n_envs)]
    )
...
def evaluate(args: argparse.Namespace, model_path: Path) -> None:
    ...
    def _rollout(name: str, decide_fn) -> tuple[str, float, int, float]:
        env = TareCarbonEnv(cfg, trace=trace, seed=args.seed)
        obs, _ = env.reset()
        cur = cfg.min_gpus
        sim_t = 0.0
        energy = 0.0
...
def main() -> None:
    ...
    parser.add_argument("--output", default="artifacts/ppo_tare.zip")
