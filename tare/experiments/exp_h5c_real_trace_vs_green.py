...
from tare.energy.carbon_trace import load_electricitymaps_csv
...
def run_zone(csv_path: Path, zone: str, tare_threshold_multiplier: float) -> list[RealTracePolicyResult]:
    ...
    tare_off_mask = tare_threshold_mask(intensities, tare_threshold_multiplier)
    tare_off_pf = pause_fraction(tare_off_mask)
    tare_off_e, tare_off_em = _evaluate_mask(tare_off_mask, intensities, sample_minutes)

    tare_on_mask = tare_threshold_online_mask(
        intensities, tare_threshold_multiplier, window_size=window_ticks,
    )
    tare_on_pf = pause_fraction(tare_on_mask)
    tare_on_e, tare_on_em = _evaluate_mask(tare_on_mask, intensities, sample_minutes)

    green_off_mask = green_offline_optimal_mask(intensities, pause_fraction=tare_off_pf)
    ...
    green_on_mask = green_online_percentile_mask(
        intensities, pause_fraction=tare_on_pf, window_size=window_ticks,
    )
    ...
    return [
        ...
        RealTracePolicyResult(zone, "tare-offline", n, sum(tare_off_mask),
                              tare_off_pf, tare_off_e, tare_off_em),
        RealTracePolicyResult(zone, "tare-online", n, sum(tare_on_mask),
                              tare_on_pf, tare_on_e, tare_on_em),
...
def main() -> int:
    ...
    parser.add_argument("--tare-threshold-multiplier", type=float, default=1.10)
    ...
    console.print(
        ...
        f"HASAGI threshold = median × {args.tare_threshold_multiplier}"
    )
    ...
    for zone, path in pairs:
        ...
        all_results.extend(run_zone(path, zone, args.tare_threshold_multiplier))
    ...
    for zone, _ in pairs:
        ...
        h_on = _pct_savings(by_zone_policy[(zone, "tare-online")].emissions_g, const_em)
        g_on = _pct_savings(by_zone_policy[(zone, "green-online")].emissions_g, const_em)
        h_off = _pct_savings(by_zone_policy[(zone, "tare-offline")].emissions_g, const_em)
        ...
        table.add_row(
            zone,
            f"{by_zone_policy[(zone, 'tare-online')].n_ticks}",
            f"{by_zone_policy[(zone, 'tare-online')].pause_fraction * 100:.1f}%",
            f"{h_on:+.2f}",
            f"{g_on:+.2f}",
            f"{h_off:+.2f}",
            f"{g_off:+.2f}",
    ...
    summary.add_row("HASAGI-online mean savings",
                    f"{statistics.mean([_pct_savings(by_zone_policy[(z, 'tare-online')].emissions_g, by_zone_policy[(z, 'constant-N')].emissions_g) for z, _ in pairs]):+.2f}%")
