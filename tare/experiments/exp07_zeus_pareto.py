...
def schedule_tare_eb(jobs: list[Job], available_gpus: int) -> dict[str, int]:
...
def run(jobs: list[Job], available_gpus: int, console: Console) -> None:
    ...
    tare_alloc = schedule_tare_eb(jobs, available_gpus)
    he, hjct, hmet = _summary(tare_alloc, jobs)
    console.print(
        f"\n[bold]HASAGI EB operating point[/]: "
        f"alloc={tare_alloc}, Σ kWh={he:.4f}, max JCT={hjct:.1f}s, "
