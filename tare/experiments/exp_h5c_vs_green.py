...
def run_zone_seed(
    ...
    days: int,
    sample_minutes: int,
    tare_threshold_multiplier: float,
) -> list[PolicyResult]:
    ...
    return [
        ...
        PolicyResult(zone, seed, "tare-offline", sum(tare_off_mask), tare_off_pf,
                     tare_off_e, tare_off_em),
        PolicyResult(zone, seed, "tare-online", sum(tare_on_mask), tare_on_pf,
                     tare_on_e, tare_on_em),
...
def main() -> int:
    ...
    for zone in args.zones:
        for seed in args.seeds:
            all_results.extend(run_zone_seed(
                ...
                args.tare_threshold_multiplier,
            ))
    ...
    fair_wins = 0
    for zone in args.zones:
        h_on = saves_for(zone, "tare-online")
        g_on = saves_for(zone, "green-online")
        h_off = saves_for(zone, "tare-offline")
        ...
        if mean_pp >= 0:
            fair_wins += 1
        pause_pcts = [
            by_zone_policy_seed[(zone, "tare-online", s)].pause_fraction * 100
            for s in args.seeds
        ]
    ...
    summary.add_row("HASAGI-online mean savings", f"{grand_mean('tare-online'):+.2f}%")
    ...
    for zone in args.zones:
        for s in args.seeds:
            h = _pct_savings(
                by_zone_policy_seed[(zone, "tare-online", s)].emissions_g,
                by_zone_policy_seed[(zone, "constant-N", s)].emissions_g,
            )
            g = _pct_savings(
