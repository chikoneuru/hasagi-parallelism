...
Usage::
    ...
    python -m experiments.exp_hasagi_eb_tight_budget --seeds 10 \\
        --out artifacts/hasagi_eb_tight_budget.json
"""
...
def main() -> int:
    ...
    parser.add_argument("--out", default="artifacts/hasagi_eb_tight_budget.json")
    ...
    for mult in args.multipliers:
        hasagi = [r.energy_kwh for r in by_key[(mult, "HASAGI EB")]]
        pf = [r.energy_kwh for r in by_key[(mult, "PowerFlow")]]
        delta_abs = statistics.mean(hasagi) - statistics.mean(pf)
        delta_pct = 100.0 * delta_abs / max(statistics.mean(pf), 1e-9)
        d = cohens_d(hasagi, pf)
