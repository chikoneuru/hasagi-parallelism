"""Tight-budget sensitivity sweep for HISE EnergyBudgetMSS.
...
wins by 9.3% on raw kWh while HISE EB wins on max JCT and deadlines
...
workload, then set the HISE EB per-job budget to
``(multiplier × E_pf) / n_jobs``. At ``multiplier = 1.0`` HISE EB has
...
At each multiplier we run HISE EB, PowerFlow, and ElasticFlow on the
...
entirely — that is precisely the contribution HISE EB exposes.
...
Two complementary views are reported:
  ...
  - **Energy-axis Cohen's d**: HISE EB vs PowerFlow at each
    multiplier, with Bonferroni-corrected α across the multiplier
    sweep.

Expected behaviour: at multipliers < 1.0 HISE EB rejects infeasible
...
energy-budget knob HISE EB exposes that PowerFlow lacks.

Usage::

    python -m experiments.exp_hise_eb_tight_budget --seeds 10 \\
        --out artifacts/hise_eb_tight_budget.json
"""
...
def _set_cluster_relative_budget(
    ...
    Infinity multiplier disables the budget gate entirely (HISE EB never
...
def main() -> int:
    ...
    parser.add_argument("--out", default="artifacts/hise_eb_tight_budget.json")
    args = parser.parse_args()

    console = Console()
    console.print(
        f"[bold]HISE EB tight-budget sweep[/]: {args.seeds} seeds × "
    ...
    # Pairwise HISE EB vs PowerFlow at each multiplier.
    n_compare = len(args.multipliers)
    bonferroni_alpha = 0.05 / max(1, n_compare)
    pairwise = Table(
        title=f"HISE EB vs PowerFlow at each multiplier — Cohen's d on energy "
              f"(Bonferroni α = {bonferroni_alpha:.4f})"
    )
    pairwise.add_column("budget ×")
    pairwise.add_column("Δ HISE − PF (kWh)", justify="right")
    pairwise.add_column("Δ %", justify="right")
    pairwise.add_column("Cohen's d", justify="right")
    pairwise.add_column("HISE met − PF met", justify="right")
    for mult in args.multipliers:
        hise = [r.energy_kwh for r in by_key[(mult, "HISE EB")]]
        pf = [r.energy_kwh for r in by_key[(mult, "PowerFlow")]]
        delta_abs = statistics.mean(hise) - statistics.mean(pf)
        delta_pct = 100.0 * delta_abs / max(statistics.mean(pf), 1e-9)
        d = cohens_d(hise, pf)
        h_met = statistics.mean(r.deadlines_met for r in by_key[(mult, "HISE EB")])
