...
layered on top by ``hasagi.energy.carbon_trace`` / ``carbon_sources``, never
...
class PrometheusPusher:
    ...
    Metrics exposed:
        hasagi_power_draw_w{worker, stage, gpu_type}    — gauge
        hasagi_throughput_iters_per_s{worker, stage}    — gauge
        hasagi_energy_cumulative_kwh{worker, stage}     — counter (monotone)
        hasagi_memory_used_bytes{worker, stage}         — gauge
        hasagi_temperature_c{worker, stage}             — gauge
    """
    ...
    def __init__(
        ...
        self._power = Gauge("hasagi_power_draw_w", "GPU/CPU power draw (W)",
                            labels, registry=registry)
        self._throughput = Gauge("hasagi_throughput_iters_per_s",
                                  "Worker iteration rate (iters/s)",
                                  labels_short, registry=registry)
        self._energy = Gauge("hasagi_energy_cumulative_kwh",
                              "Cumulative energy (kWh) since job start",
                              labels_short, registry=registry)
        self._memory = Gauge("hasagi_memory_used_bytes", "GPU memory used (bytes)",
                              labels_short, registry=registry)
        self._temp = Gauge("hasagi_temperature_c", "GPU temperature (C)",
                            labels_short, registry=registry)
