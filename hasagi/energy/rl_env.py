...
core ``hasagi`` install stays light. Training scripts import this module; the
orchestrator imports only the lightweight scaffold in ``hasagi.energy.rl_policy``.
"""
...
except ImportError as exc:  # pragma: no cover — install hasagi[rl] to use this module.
    raise ImportError(
        "hasagi.energy.rl_env requires gymnasium; install hasagi[rl]"
    ) from exc

from hasagi.energy.carbon_trace import CarbonTrace, synthetic_solar_trace
