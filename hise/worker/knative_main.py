...
logger = logging.getLogger("hise.worker.knative")
logging.basicConfig(
    level=os.environ.get("HISE_WORKER_LOGLEVEL", "INFO"),
...
CUDA_INIT_S = float(os.environ.get("HISE_MOCK_CUDA_INIT_S", "3.0"))
# Per-request "work" cost in seconds; mimics a single training iteration.
WORK_ITER_S = float(os.environ.get("HISE_MOCK_WORK_ITER_S", "0.05"))
...
def main() -> None:
    import uvicorn
    uvicorn.run(
        "hise.worker.knative_main:app",
        host="0.0.0.0",   # noqa: S104 — container intentionally binds all
        port=int(os.environ.get("PORT", "8080")),
        log_level=os.environ.get("HISE_WORKER_LOGLEVEL", "info").lower(),
        access_log=False,
    )
