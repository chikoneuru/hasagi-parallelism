...
core ``hise`` install stays light. Training scripts import this module; the
orchestrator imports only the lightweight scaffold in ``hise.energy.rl_policy``.
"""
...
except ImportError as exc:  # pragma: no cover — install hise[rl] to use this module.
    raise ImportError(
        "hise.energy.rl_env requires gymnasium; install hise[rl]"
    ) from exc

from hise.energy.carbon_trace import CarbonTrace, synthetic_solar_trace
